...
patchocc <- patchocc[which(!is.na(list_pixel[,1])),,] # redefine the occupanycy table with only the prospected sites
...
se_psiAb <- deltamethod(g2, x2, hessian[2,2], ses=TRUE) 
...
g1<-expression(exp(x1) / (1+exp(x1)+exp(x2)+exp(x3)))
se_nuA <- se_epsilonB # extinction of poachers in absence of wildlife is the same as extinction of poacher in presence of wildlife 
...
se_epsilonAB, 
se_epsilonB,
se_epsilonA, 5se_pAb, se_pAB, se_pB) 
#SE 
```
...
```{r}
 ...
 seAB<-expand.grid(se_pAB)
  ...
  mat3 <- data.frame( x=x, estimates = pAB, sem= pAB - 0.01, sep= pAB + 0.01, parameter= rep("p_WP", length(x)))
