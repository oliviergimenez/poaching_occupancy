...
se_psiAb <- deltamethod(g1, x1, hessian[1,1], ses=TRUE)

g3<-expression(exp(x3) / (1+exp(x3)))

se_psiAB <- se_psiAb # occupancy of Poachers regardless of wildlife occurence
...
se_etaA <- se_gammaA # colonisation by poacher regardless of presence/absence of wildlife 
...
names(AICtrans)<-c("poachers performance on transition", "no prey effects on transition",  "prey defensive strategy on transition", "spatial effects of patrolling effort on transition of poachers only", "temporal effects of patrolling effort on transition of poachers only", "spatial effects of patrolling effort on transition", "temporal effects of patrolling effort on transition", "spatio-temporal effects of patrolling effort on transition", "delayed effects of spatio-temporal patrolling effort on transition")
...
Table 1 : Model selection considering effects of species interactions, sampling and environmental variations on detection, occupancy and transition probabilities of poachers and wildlife. The deviance, QAICc the quasi-Akaike Information Criterion corrected for small sample size and overdispersed data. The best model supported by the data is the one with the lowest AICc
...
moy<-c(mle_psiB, mle_psiAb, mle_epsilonB[[2]], mle_epsilonA[[2]], mle_nuB[[2]], mle_nuA[[2]],  mle_gammaB[[2]], mle_gammaA[[2]], mle_etaB[[2]], mle_etaA[[2]], mle_omegaBA[[2]], mle_omegaAB[[2]]) 
...
## Detection probabilities and their associated confidence intervals as a function of patrolling effort  
```{r}
 ...
 seAB<-expand.grid(se_pAB)
  ...
  mat1 <- data.frame( x=x, estimates = pB, sem= pB - seB, sep= pB + seB, parameter= rep("p_W", length(x)))
...
## Probability of being in the 4 occupancy state  over 5 years.
As a reminder, the sites can be in the following states : Unoccupied, occupied by wildlife only WO, by poachers only PO, by both poachers and wildlife PW
```{r}
par(mfrow=c(2,2))
...
epsilonA<-rnorm(MCiter, mle_epsilonA[[2]], se_epsilonA)
...
epsilonB<-rnorm(MCiter, mle_epsilonB[[2]], se_epsilonB)
epsilonB[epsilonB<0]<-0
epsilonB[epsilonB>1]<-1
trans42<-epsilonB


nuA<-rnorm(MCiter, mle_nuA[[2]], se_nuA)
nuA[nuA<0]<-0
nuA[nuA>1]<-1



nuB<-rnorm(MCiter, mle_nuB[[2]], se_nuB)
nuB[nuB<0]<-0
nuB[nuB>1]<-1


etaA<-rnorm(MCiter, mle_etaA[[2]], se_etaA)
etaA[etaA<0]<-0
etaA[etaA>1]<-1


etaB<-rnorm(MCiter, mle_etaB[[2]], se_etaB)
etaB[etaB<0]<-0
etaB[etaB>1]<-1


gammaB<-rnorm(MCiter, mle_gammaB[[2]], 0.01)
gammaB[gammaB<0]<-0
gammaB[gammaB>1]<-1
trans13<-gammaB

gammaA<-rnorm(MCiter, mle_gammaA[[2]], se_gammaA)
gammaA[gammaA<0]<-0
gammaA[gammaA>1]<-1
trans12<-gammaA

omegaAB<-rnorm(MCiter, mle_omegaAB[[2]], se_omegaAB)
omegaAB[omegaAB<0]<-0
omegaAB[omegaAB>1]<-1


omegaBA<-rnorm(MCiter, mle_omegaBA[[2]], se_omegaBA)
omegaBA[omegaBA<0]<-0
omegaBA[omegaBA>1]<-1
...
{  
  ...
  #
     ...
     {  
      ...
      mle_epsilonAB[[2]],trans42[i],trans43[i],1-trans42[i]-trans43[i]-mle_epsilonAB[[2]]), nrow = 4, byrow = TRUE)
  ...
  geom_line(data=mat,aes(x=x, y=psiB)) + 
   ...
   p + theme_classic() 
    ...
    colnames(meandynmat)<-c("U", "PO", "WO", "WP")
   meandynmat
```
